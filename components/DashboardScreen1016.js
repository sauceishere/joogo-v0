...
////////// if New User, then go to Profile.js for FIRST fill out ////////////////////////////
  ...
  _makeRemoteRequest = async () => {
    ...
    const _loadDashboardFlatlist = (idTokenCopied) => {
      ...
      fetch('https://asia-northeast1-joogo-v0.cloudfunctions.net/loadDashboardFlatlist1016-py', { // https://developer.mozilla.org/ja/docs/Web/API/Fetch_API/Using_Fetch
        method: 'POST',
        headers: {
          // 'Accept': 'application/json', 
          'Content-Type' : 'application/json' // text/html text/plain application/json
  ...
  render() {
    console.log('---------------- render');
    const { isLoading, const_exer, scaler_scale, scaler_mean, model, vidViewLogTemp, nname, wval, wunit, hval, hunit, nat, byr, gdr, bt0, bt1, ts, llogin, lupdate, isGuest, model2, adUnitID, post } = this.state; // scoreTtl, playSumTtl, playCnt, dataByYearWeeks, StatsDataLoadedAt, lastPlayEnded
